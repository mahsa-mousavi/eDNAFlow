...
dada2_script = file(params.dada2_script)
cutadapt_script = file(params.cutadapt_script)
...
if (params.taxonomyAssignment) {
  ...
  phyloseq_script = file(params.phyloseq_script)
	(zotuTable_ch_a, zotuTable_ch_b) = Channel.fromPath(params.zotuTable).into(2)
	blastFile_ch = Channel.fromPath(params.blastFile)
  fastaFile_ch = Channel.fromPath(params.fastaFile)
  metadataFile_ch = Channel.fromPath(params.metadataFile)
	lca_qcov = params.lca_qcov
	lca_pid = params.lca_pid
	lca_diff = params.lca_diff
	lca_output = params.lca_output
} else {
	zotuTable_ch_a = Channel.empty()
  zotuTable_ch_b = Channel.empty()
	blastFile_ch = Channel.empty()
  fastaFile_ch = Channel.empty()
  metadataFile_ch = Channel.empty()
	lca_script = Channel.empty()
  phyloseq_script = Channel.empty()
}
...
process '09_taxonomyAssigned' {
  ...
  input:
    tuple path(table), path(blastRes) from zotuTable_ch_a.combine(blastFile_ch)
    file lcaScript from lca_script

  output:
    tuple path("interMediate_res.tab"), path("${lca_output}_qCov${lca_qcov}_id${lca_pid}_diff${lca_diff}.tab") into lca_ch
...
/*
 * Process 10: Create phyloseq object
 */
process '10_create_phyloseq' {
  ...
  input:
    ...
    path(zotu_table) from zotuTable_ch_b
